...
impl Database {
    ...
    .map_err(|e| format!("Failed to insert review event: {e}"))?;
      ...
      ON CONFLICT(prompt_id) DO UPDATE SET
        ...
        drop(conn);
        self.get_review_schedule(&schedule.prompt_id)?
    ...
    fn recent_review_events_are_ordered_bounded_and_ignore_orphans() {
...

        {
            ...
            for (id, prompt_id, reviewed_at) in [
                ...
                conn.execute(
                    "INSERT INTO review_events (id, prompt_id, reviewed_at, outcome, duration_ms, user_response, provenance)
                     VALUES (?1, ?2, ?3, 'good', 1000, 'answer', 'user_authored')",
            ...
            conn.execute(
                "INSERT INTO review_events (id, prompt_id, reviewed_at, outcome, duration_ms, user_response, provenance)
                 VALUES ('event-orphan', 'missing-prompt', '2026-08-21T11:00:00Z', 'good', 1000, 'answer', 'user_authored')",
                [],
            )
            .unwrap();
