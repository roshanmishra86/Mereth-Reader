...
impl Database {
    ...
    pub fn get_current_version(
        &self,
        document_id: &str,
    ) -> Result<Option<DocumentVersion>, String> {
    ...
    pub fn check_document_version_state(
        &self,
        document_id: &str,
    ...
    .map_err(|e| e.to_string())?;
        ...
        let persisted = self.get_current_version(document_id)?.unwrap_or(version);
        Ok(persisted)
    }
    ...
    pub fn update_version_geometry(
      ...
      .map_err(|e| e.to_string())?;
        ...
        conn.execute(
            ...
            params![
                serde_json::to_string(geometry).map_err(|e| e.to_string())?,
                version_id
            ],
        )
        .map_err(|e| e.to_string())?;
    ...
    pub fn reanchor_annotation_to_version(
        ...
        if annotation_doc != version_doc {
            return Err(
                "Cannot re-anchor an annotation to a version of another document".to_string(),
            );
        }
...
mod tests {
    use super::*;
    use crate::db::{
        annotations::{Annotation, NormalizedRect},
        Database, Document,
    };
    use std::io::Write;
    ...
    fn write_pdf(path: &std::path::Path, marker: &str) {
        ...
        f.write_all(format!("%PDF-1.4 {marker} content here").as_bytes())
            .unwrap();
        f.flush().unwrap();
    }
    ...
    fn test_version_lifecycle_registers_v1_then_v2_on_byte_change() {
        ...
        assert_eq!(
            again.id, v1.id,
            "same bytes must not create a duplicate version"
        );
        ...
        assert_ne!(
            check.file_sha256_hash.as_deref(),
            Some(v1.sha256_hash.as_str())
        );
    ...
    fn test_reanchor_moves_annotation_between_versions_without_touching_content() {
        ...
        assert!(db
            .reanchor_annotation_to_version(&annotation.id, &v2.id, "")
            .is_err());

        db.reanchor_annotation_to_version(&annotation.id, &v2.id, "checksum-v2")
            .unwrap();
        ...
        assert!(db
            .reanchor_annotation_to_version(&annotation.id, &v2.id, "again")
            .is_err());
        ...
        assert!(db
            .reanchor_annotation_to_version(&other_ann.id, &v2.id, "zzz")
            .is_err());
    }
}
