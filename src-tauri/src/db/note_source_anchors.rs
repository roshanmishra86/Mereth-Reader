...
impl Database {
  pub fn create_quick_note(&self, note: &Note, anchor: &NoteSourceAnchor) -> Result<QuickNoteTransactionResult, String> {
    if note.note_type != "scratch" || note.id != anchor.note_id || note.document_id.as_deref() != Some(anchor.document_id.as_str()) {
    ...
    let page_count: i64 = tx.query_row(
        ...
        |row| row.get(0),
    ).map_err(|_| "Document version not found or does not belong to the document".to_string())?;
    ...
    Ok(QuickNoteTransactionResult { note: note.clone(), source_anchor: anchor.clone() })
  }

  pub fn list_note_source_anchors(&self, document_id: &str) -> Result<Vec<NoteSourceAnchor>, String> {
    ...
    let rows = stmt.query_map(params![document_id], |r| Ok(NoteSourceAnchor { id:r.get(0)?,note_id:r.get(1)?,document_id:r.get(2)?,document_version_id:r.get(3)?,page_index:r.get(4)?,page_label:r.get(5)?,selected_quote:r.get(6)?,rects_json:r.get(7)?,created_at:r.get(8)?,provenance:r.get(9)? })).map_err(|e| e.to_string())?;
