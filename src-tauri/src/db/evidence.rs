...
impl Database {
    ...
    pub fn capture_annotation_to_source_note(
      ...
      map_row_to_evidence,
        ...
        if let Some(evidence_block) = existing_block {
            tx.commit().map_err(|e| e.to_string())?;
            return Ok(CaptureToSourceNoteResult {
                note,
                evidence_block,
            });
        }
    ...
    pub fn update_evidence_block_comment(
        &self,
        id: &str,
        user_comment: &str,
    ) -> Result<(), String> {
    ...
    pub fn list_evidence_blocks(
        &self,
        note_id: &str,
    ) -> Result<Vec<AttachedEvidenceBlock>, String> {
        let conn = self.conn.lock().map_err(|e| e.to_string())?;
        let mut stmt = conn
      .prepare(
...
pub mod tests {
    ...
    fn test_evidence_block_comment_update_and_reordering() {
        ...
        // Update comment
        db.update_evidence_block_comment("eb-10", "Updated comment text")
            .unwrap();
        let list = db.get_note_evidence_blocks(&note_id).unwrap();
        assert_eq!(list[0].user_comment, "Updated comment text");

        // Reorder: swap order
        db.update_evidence_block_order(&note_id, &["eb-20".to_string(), "eb-10".to_string()])
            .unwrap();
    ...
    fn test_capture_annotation_to_source_note_creates_source_note_and_evidence_atomically() {
        ...
        let doc = crate::db::Document {
            ...
            sha256_hash: "1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd"
                .to_string(),
            page_count: 5,
            created_at: "2026-08-21T00:00:00Z".to_string(),
            updated_at: "2026-08-21T00:00:00Z".to_string(),
        ...
        let captured = db
            .capture_annotation_to_source_note(&annotation_block)
            .unwrap();
        annotation_block.id = "annotation-capture-retry".into();
        assert_eq!(
            db.capture_annotation_to_source_note(&annotation_block)
                .unwrap(),
            captured
        );
