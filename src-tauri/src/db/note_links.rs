...
impl Database {
    ...
    pub fn add_note_link(&self, link: &NoteLink) -> Result<NoteLink, String> {
        ...
        if target_count != 1 {
            return Err(
                "A note link must specify exactly one target (note, document, or annotation)"
                    .to_string(),
            );
        }
        ...
        tx.execute(
            &format!(
                "INSERT INTO note_links ({LINK_COLS}) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)"
            ),
            params![
                link_id,
                link.note_id,
...
pub mod tests {
    ...
    fn test_sync_note_links() {
        ...
        db.sync_note_links(&note_a, &[note_b.clone()], &[], &[])
            .unwrap();
