    ...
    if current_version < LATEST_MIGRATION_VERSION {
       ...
       VALUES (1, datetime('now'), 'migration_1_init_v1');",
        ...
        if current_version < 2 {
            ...
            tx.execute(
                "ALTER TABLE documents ADD COLUMN collections TEXT NOT NULL DEFAULT '[]';",
                [],
            )?;
       ...
       VALUES (3, datetime('now'), 'migration_3_reading_sessions');",
        ...
        [],
            ...
            tx.execute(
                "CREATE INDEX idx_annotations_trash ON annotations(deleted_at);",
                [],
            )?;
       ...
       VALUES (5, datetime('now'), 'migration_5_notes_revisions_links');",
        ...
        if current_version < 6 {
          ...
          provenance TEXT NOT NULL CHECK(provenance IN (
            ...
            tx.execute(
                "CREATE INDEX idx_evidence_blocks_note ON evidence_blocks(note_id);",
                [],
            )?;
            tx.execute(
                "CREATE INDEX idx_evidence_blocks_annotation ON evidence_blocks(annotation_id);",
                [],
            )?;
       ...
       VALUES (6, datetime('now'), 'migration_6_evidence_blocks');",
        ...
        if current_version < 7 {
          ...
          CHECK (annotation_id IS NOT NULL OR note_id IS NOT NULL)
            ...
            tx.execute(
                "CREATE INDEX idx_review_prompts_annotation ON review_prompts(annotation_id);",
                [],
            )?;
            tx.execute(
                "CREATE INDEX idx_review_prompts_note ON review_prompts(note_id);",
                [],
            )?;
            tx.execute(
                "CREATE INDEX idx_review_prompts_status ON review_prompts(status);",
                [],
            )?;
       ...
       VALUES (11, datetime('now'), 'migration_11_annotation_checksum');",
        ...
        FROM annotations;",
            ...
            tx.execute(
                "CREATE INDEX IF NOT EXISTS idx_annotations_trash ON annotations(deleted_at);",
                [],
            )?;
       ...
       VALUES (12, datetime('now'), 'migration_12_imported_markup_empty_quote');",
        ...
        [], |row| row.get(0),
          ...
          [],
            ...
            if !has_original {
                tx.execute(
                    "ALTER TABLE documents ADD COLUMN original_filepath TEXT;",
                    [],
                )?;
            }
            if !has_removed {
                tx.execute("ALTER TABLE documents ADD COLUMN removed_at TEXT;", [])?;
            }
        ...
        [], |row| row.get(0),
            ...
            if !has_version_hash {
                tx.execute(
                    "ALTER TABLE pages ADD COLUMN version_hash TEXT NOT NULL DEFAULT '';",
                    [],
                )?;
            }
      ...
      CREATE INDEX idx_note_source_anchors_document_page ON note_source_anchors(document_id,page_index);")?;
        ...
        fn parse_cloze_indices_for_migration(
        ...
        [],
          ...
          "INSERT INTO review_schedule_v16 (
            ...
            due_at, last_reviewed_at, last_outcome, fsrs_version, updated_at, provenance, original_provenance
                ...
                {
                    let indices = parse_cloze_indices_for_migration(
                        prompt_type.as_deref(),
                        question.as_deref(),
                    );
          ...
          "UPDATE review_events SET cloze_index = ?1 WHERE prompt_id = ?2 AND cloze_index = 0",
            ...
            tx.execute(
                "ALTER TABLE review_schedule_v16 RENAME TO review_schedule;",
                [],
            )?;
            tx.execute(
