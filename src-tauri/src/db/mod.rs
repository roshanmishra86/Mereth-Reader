...
fn default_ownership_mode() -> String {
    "open_in_place".into()
}
...
fn relocate_legacy_database(app_dir: &Path, new_db_path: &Path) -> Result<(), String> {
    ...
    let db_dir = new_db_path.parent().ok_or("Invalid database directory")?;
...
impl Database {
    ...
    pub fn get_documents(&self) -> Result<Vec<Document>, String> {
        ...
        let query = format!(
            "SELECT {} FROM documents WHERE removed_at IS NULL",
            DOCUMENT_SELECT_COLS
        );
    ...
    pub fn add_document(&self, doc: Document) -> Result<(), String> {
        ...
        let collections_json =
            serde_json::to_string(&doc.collections).unwrap_or_else(|_| "[]".into());
    ...
    pub fn get_document_by_hash(&self, sha256_hash: &str) -> Result<Option<Document>, String> {
        let conn = self.conn.lock().unwrap();
        let query = format!(
            "SELECT {} FROM documents WHERE sha256_hash = ?1 LIMIT 1",
            DOCUMENT_SELECT_COLS
        );
        let mut stmt = conn.prepare(&query).map_err(|e| e.to_string())?;
    ...
    pub fn remove_document(&self, id: &str) -> Result<(), String> {
      ...
      params![id],
        ...
        if changed == 0 {
            return Err(format!("Document not found or already removed: {id}"));
        }
        Ok(())
    }
    ...
    pub fn restore_document(&self, id: &str) -> Result<(), String> {
      ...
      params![id],
        ...
        if changed == 0 {
            return Err(format!("Removed document not found: {id}"));
        }
        Ok(())
    }
    ...
    pub fn get_removed_documents(&self) -> Result<Vec<Document>, String> {
        let conn = self.conn.lock().unwrap();
        let query = format!(
            "SELECT {} FROM documents WHERE removed_at IS NOT NULL ORDER BY removed_at DESC",
            DOCUMENT_SELECT_COLS
        );
        let mut stmt = conn.prepare(&query).map_err(|e| e.to_string())?;
        let documents = stmt
    ...
    pub fn get_pages_for_version(
      ...
      "SELECT id, document_id, page_number, width, height, text_content, created_at, provenance
       FROM pages WHERE document_id = ?1 AND version_hash = ?2 ORDER BY page_number ASC",
        ...
        let pages = stmt
            .query_map(params![document_id, version_hash], |row| {
                Ok(Page {
                    id: row.get(0)?,
                    document_id: row.get(1)?,
                    page_number: row.get(2)?,
    ...
    /// U20: drops all cached page text for one document version so indexing can
    /// rebuild from scratch (corrupt-cache recovery path).
    pub fn delete_pages_for_version(
        &self,
        document_id: &str,
        version_hash: &str,
    ) -> Result<usize, String> {
    ...
    pub fn upsert_pages_for_version(
        &self,
        pages: Vec<Page>,
        version_hash: &str,
    ) -> Result<(), String> {
    ...
    pub fn get_jobs(&self) -> Result<Vec<Job>, String> {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, job_type, status, payload, error, created_at, updated_at FROM jobs",
            )
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn test_fts5_indexing_and_rebuild() {
        ...
        let page = Page {
            ...
            width: 612.0,
            height: 792.0,
            text_content: "Plaintiffs motion for summary judgment regarding breach of contract"
                .into(),
            created_at: "2026-08-04T13:52:57Z".into(),
            provenance: "source_extracted".into(),
        };
    ...
    fn test_versioned_page_text_cache_isolated_and_upserted() {
        ...
        db.add_document(Document {
            id: doc_id.clone(),
            title: "Versioned cache".into(),
            filepath: "/tmp/cache.pdf".into(),
            sha256_hash: "hash-v2".into(),
            page_count: 1,
        ...
        db.upsert_page_for_version(make_page("v1-page", "old bytes"), "hash-v1")
            .unwrap();
        db.upsert_page_for_version(make_page("v2-page", "new bytes"), "hash-v2")
            .unwrap();
        db.upsert_page_for_version(make_page("v2-page-retry", "new bytes refreshed"), "hash-v2")
            .unwrap();

        assert_eq!(
            db.get_pages_for_version(&doc_id, "hash-v1").unwrap()[0].text_content,
            "old bytes"
        );
        ...
        assert!(
            db.search_fts("old").unwrap().is_empty(),
            "rebuild must exclude stale source versions"
        );
        assert_eq!(db.search_fts("refreshed").unwrap().len(), 1);

        // Corrupt-cache recovery is tightly scoped: a request for a different
        // document cannot evict this document's old version, and evicting the
        // old version leaves the current version intact.
        assert_eq!(
            db.delete_pages_for_version("other-document", "hash-v1")
                .unwrap(),
            0
        );
        assert_eq!(
            db.get_pages_for_version(&doc_id, "hash-v1").unwrap().len(),
            1
        );
        assert_eq!(db.delete_pages_for_version(&doc_id, "hash-v1").unwrap(), 1);
        assert!(db
            .get_pages_for_version(&doc_id, "hash-v1")
            .unwrap()
            .is_empty());
        assert_eq!(
    ...
    fn test_backup_created_before_migration_of_existing_db() {
        ...
        assert!(
            backup_path.exists(),
            "backup must be created before a migration runs on an existing db"
        );
    }
    ...
    fn test_document_metadata_and_collections() {
        ...
        db.rename_collection(&col_id, "Advanced Legal Research")
            .unwrap();
    ...
    fn test_remove_restore_preserves_document_and_linked_rows() {
        ...
        db.add_document(Document {
            ...
            filepath: "/docs/recoverable.pdf".into(),
            ...
            tags: vec![],
            collections: vec![],
            ownership_mode: "managed_library".into(),
            original_filepath: Some("/source/recoverable.pdf".into()),
            removed_at: None,
        })
        .unwrap();
        db.add_page(Page {
        ...
        assert_eq!(
            removed[0].original_filepath.as_deref(),
            Some("/source/recoverable.pdf")
        );
        assert_eq!(db.get_pages(&doc_id).unwrap().len(), 1);
    ...
    fn test_database_lives_in_db_subdirectory_and_relocates_legacy() {
        ...
        let legacy_path = dir2.path().join("mereth_reader.db");
        {
            ...
            .unwrap();
            conn.execute("INSERT INTO legacy_probe (note) VALUES ('still here')", [])
                .unwrap();
            drop(conn);
        }
        {
            ...
            let note: String = conn
                .query_row("SELECT note FROM legacy_probe WHERE id = 1", [], |r| {
                    r.get(0)
                })
                .unwrap();
    ...
    fn test_reopening_version3_database_runs_forward_migrations_with_backup() {
        ...
        let db_path = dir.path().join("db").join("mereth_reader.db");
        {
            ...
            .unwrap();
            conn.execute("DELETE FROM migration_metadata WHERE version > 3", [])
                .unwrap();
            drop(conn);
        }
    ...
    fn test_adoption_never_erases_original_provenance_at_schema_level() {
      ...
      [],
        ...
        assert!(
            res.is_err(),
            "user_adopted_ai without an original must fail the CHECK"
        );
      ...
      [],
        ...
        assert!(
            res.is_err(),
            "re-adoption that erases the first original must fail the CHECK"
        );
      ...
      [],
        ...
        assert!(
            res.is_err(),
            "original_provenance outside the six values must fail the CHECK"
        );
      ...
      .unwrap();
        let original_null: Option<String> = conn
            .query_row(
                "SELECT original_provenance FROM notes WHERE id = 'n5'",
                [],
                |r| r.get(0),
            )
            .unwrap();
      ...
      [],
        ...
        assert!(
            res.is_err(),
            "provenance outside the six values must fail the CHECK"
        );
    }
}
