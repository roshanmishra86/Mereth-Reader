...
impl Database {
    ...
    pub fn split_note_transaction(
        ...
        if new_note.note_type != "concept"
            || link.note_id != original_id
            || link.target_note_id.as_deref() != Some(new_note.id.as_str())
        {
        ...
        tx.execute(
            ...
            params![original_title, original_body, now, original_id],
        )
        .map_err(|e| format!("Failed to update original note: {e}"))?;
        let revision_number: i64 = tx
            .query_row(
                "SELECT COALESCE(MAX(revision_number), 0) + 1 FROM note_revisions WHERE note_id=?1",
                params![original_id],
                |r| r.get(0),
            )
        ...
        tx.execute(
            &format!(
                "INSERT INTO notes ({NOTE_COLS}) VALUES (?1, ?2, ?3, ?4, ?5, NULL, ?6, ?6, ?7, ?8)"
            ),
            params![
                new_note.id,
                new_note.note_type,
                new_note.title,
                new_note.body_markdown,
                new_note.document_id,
                now,
                new_note.provenance,
                new_note.original_provenance
            ],
        )
        .map_err(|e| format!("Failed to create split note: {e}"))?;
        ...
        Ok(SplitNoteTransactionResult {
            original_note: Note {
                ...
                body_markdown: original_body.to_string(),
    ...
    pub fn update_note(
        ...
        let existing: Note = {
            let mut stmt = tx
                .prepare(&format!("SELECT {NOTE_COLS} FROM notes WHERE id = ?1"))
                .map_err(|e| e.to_string())?;
            stmt.query_row(params![id], |r| map_row_to_note(r))
    ...
    pub fn restore_note_revision(
        ...
        let target_rev: NoteRevision = {
          ...
          "SELECT {REVISION_COLS} FROM note_revisions WHERE note_id = ?1 AND revision_number = ?2"
                ...
                .map_err(|e| e.to_string())?;
            stmt.query_row(params![note_id, revision_number], |r| {
                map_row_to_revision(r)
            })
        ...
        let existing: Note = {
            let mut stmt = tx
                .prepare(&format!("SELECT {NOTE_COLS} FROM notes WHERE id = ?1"))
                .map_err(|e| e.to_string())?;
            stmt.query_row(params![note_id], |r| map_row_to_note(r))
                .map_err(|e| format!("Note not found: {e}"))?
        };
...
pub mod tests {
    ...
    fn test_note_update_and_bounded_revisions_fifo() {
        ...
        for i in 2..=26 {
            ...
            db.update_note("n3", &updated_title, &updated_body, true)
                .unwrap();
        }
    ...
    fn test_split_note_transaction_is_all_or_nothing() {
        ...
        let link = NoteLink {
            id: "split-link".to_string(),
            note_id: "original".to_string(),
            target_note_id: Some("split".to_string()),
        ...
        let result = db
            .split_note_transaction(
                "original",
                "Original",
                "See [[mereth:note/split|Split]]",
                &new_note,
                &link,
            )
            .unwrap();
        ...
        let invalid_link = NoteLink {
            target_note_id: Some("wrong-target".to_string()),
            ..link
        };
        assert!(db
            .split_note_transaction(
