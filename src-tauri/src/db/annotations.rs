...
pub const ANNOTATION_TYPES: &[&str] = &["highlight", "underline", "area", "comment", "bookmark"];
...
pub fn validate_asset_kind(kind: &str) -> Result<(), String> {
    if ASSET_KINDS.contains(&kind) {
        ...
        Err(format!(
            "Invalid asset kind '{kind}'; expected one of {ASSET_KINDS:?}"
        ))
    }
}
...
pub fn validate_annotation(annotation: &Annotation) -> Result<(), String> {
    ...
    match annotation.annotation_type.as_str() {
        ...
        "highlight" | "underline"
            if !has_quote && annotation.provenance != "deterministic_transform" =>
        {
            return Err(
                "Text highlight/underline annotations require an exact quote (FR-9.4)".to_string(),
            );
        }
...
pub fn asset_full_path(app_dir: &Path, relative_path: &str) -> Result<PathBuf, String> {
    ...
    let canonical_root = fs::canonicalize(&annotations_dir).map_err(|e| e.to_string())?;
    ...
    let canonical_parent = fs::canonicalize(parent)
        .map_err(|e| format!("Asset parent directory is not accessible: {e}"))?;
...
impl Database {
    ...
    pub fn get_all_annotations(&self, include_trashed: bool) -> Result<Vec<Annotation>, String> {
       ...
       {where_clause} ORDER BY updated_at DESC, page_index ASC",
            where_clause = if include_trashed {
                ""
            } else {
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if !known_paths.contains(&relative_path) {
                if let Err(e) = fs::remove_file(&path) {
                    ...
                    eprintln!(
                        "reconcile_orphaned_asset_files: failed to remove orphaned asset '{}': {e}",
                        path.display()
                    );
                } else {
                    removed += 1;
    ...
    pub fn add_annotation_asset(
        ...
        let full = asset_full_path(app_dir, &asset.relative_path)?;
        if !full.exists() {
            return Err(format!("Asset file does not exist at '{}'", full.display()));
        }
...
mod tests {
    ...
    fn test_annotation_roundtrip_and_listing() {
        ...
        let by_id = db
            .get_annotation_by_id(&annotation.id)
            .unwrap()
            .expect("exists");
    ...
    fn test_annotation_validation_rejects_bad_input() {
        ...
        bad_rect.rects = vec![NormalizedRect {
            x: -0.1,
            y: 0.5,
            width: 0.3,
            height: 0.3,
        ...
        overflow_rect.rects = vec![NormalizedRect {
            x: 0.9,
            y: 0.5,
            width: 0.3,
            height: 0.3,
        ...
        nan_rect.rects = vec![NormalizedRect {
            x: f64::NAN,
            y: 0.5,
            width: 0.3,
            height: 0.3,
      ...
      params![doc_id, version_id],
        ...
        assert!(
            res.is_err(),
            "annotation_type CHECK must reject 'ink' at the schema level"
        );
    }
    ...
    fn test_trash_restore_purge_lifecycle() {
        ...
        assert_eq!(
            db.get_annotations_for_document(&doc_id, false)
                .unwrap()
                .len(),
            1
        );
        ...
        let asset_path = app_dir
            .path()
            .join(ASSET_ROOT_COMPONENT)
            .join("asset-1.png");
        ...
        assert!(db
            .update_annotation_fields(&annotation.id, "red", "nope", &[])
            .is_err());
    ...
    fn test_asset_path_validation_and_lifecycle() {
        ...
        for bad in [
            ...
            assert!(
                validate_asset_relative_path(bad).is_err(),
                "must reject: {bad}"
            );
        }
        ...
        db.add_annotation_asset(app_dir.path(), &asset).unwrap();
        let fetched = db
            .get_annotation_asset_by_id("asset-real")
            .unwrap()
            .unwrap();
        ...
        db.delete_annotation_asset(app_dir.path(), "asset-real")
            .unwrap();
        assert!(db
            .get_annotation_asset_by_id("asset-real")
            .unwrap()
        ...
        assert!(db
            .delete_annotation_asset(app_dir.path(), "asset-real")
            .is_err());
    }
    ...
    fn test_asset_file_write_read_and_confinement() {
        ...
        db.write_asset_file(app_dir.path(), good_path, &bytes)
            .unwrap();
    ...
    fn test_remove_asset_file_cleanup_path() {
        ...
        db.write_asset_file(app_dir.path(), "annotations/cleanup.png", b"bytes")
            .unwrap();
        let target = app_dir.path().join("annotations").join("cleanup.png");
        assert!(target.exists());

        // Removal deletes the file; a second removal is a no-op.
        db.remove_asset_file(app_dir.path(), "annotations/cleanup.png")
            .unwrap();
        assert!(!target.exists());
        db.remove_asset_file(app_dir.path(), "annotations/cleanup.png")
            .unwrap();
        ...
        for bad in ["../escape.png", "/abs.png", "mereth_reader.db"] {
            assert!(
                db.remove_asset_file(app_dir.path(), bad).is_err(),
                "must reject: {bad}"
            );
        }
        ...
        db.remove_asset_file(app_dir.path(), "annotations/none.png")
            .unwrap();
    ...
    fn test_asset_file_write_atomicity_and_bad_paths() {
        ...
        for bad in [
            ...
            assert!(
                db.write_asset_file(app_dir.path(), bad, b"x").is_err(),
                "must reject: {bad}"
            );
        }

        // A successful write leaves no .tmp leftovers behind.
        db.write_asset_file(app_dir.path(), "annotations/clean.png", b"abc")
            .unwrap();
        ...
        assert!(
            leftovers.is_empty(),
            "atomic write must not leak temp files: {leftovers:?}"
        );

        // Overwriting an existing asset is allowed (idempotent re-capture) and
        // stays atomic.
        db.write_asset_file(app_dir.path(), "annotations/clean.png", b"xyz")
            .unwrap();
        assert_eq!(
        ...
        db.write_asset_file(app_dir.path(), "annotations/empty.png", b"")
            .unwrap();
        assert!(app_dir
            .path()
            .join("annotations")
    ...
    fn test_fk_cascades_and_checks_fire_at_schema_level() {
      ...
      params![doc_id, version_id],
        ...
        assert!(
            ok_imported.is_ok(),
            "imported highlight without quote must pass the CHECK"
        );
