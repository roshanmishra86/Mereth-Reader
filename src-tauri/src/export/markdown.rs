...
pub fn export_markdown_package(
    ...
    for note in &notes {
      ...
      note.title, note.id, note.note_type, note.created_at, note.updated_at, note.provenance
        ...
        fs::write(&note_file_path, content)
    ...
    for doc in &documents {
      ...
      doc.id, doc.title, doc.author, doc.sha256_hash, doc.doi, doc.created_at, doc.provenance, doc.title
        ...
        fs::write(&source_file_path, doc_content)
    ...
    if !prompts.is_empty() {
        ...
        fs::write(&reviews_file_path, review_content)
            .map_err(|e| format!("Failed to write reviews file: {e}"))?;
    ...
    let total_items = (manifest.notes.len()
        + manifest.sources.len()
        + manifest.assets.len()
        + manifest.reviews.len()) as i64;
    record_export(
        db,
        "markdown",
        destination_dir,
