...
fn write_journal(path: &Path, journal: &RestoreJournal) -> Result<(), String> {
    let bytes = serde_json::to_vec_pretty(journal)
        .map_err(|e| format!("Failed to encode restore journal: {e}"))?;
    let temp = path.with_extension("tmp");
    let mut file =
        fs::File::create(&temp).map_err(|e| format!("Failed to create restore journal: {e}"))?;
    use std::io::Write;
    file.write_all(&bytes)
        .map_err(|e| format!("Failed to write restore journal: {e}"))?;
    file.sync_all()
        .map_err(|e| format!("Failed to flush restore journal: {e}"))?;
...
fn reject_symlink_components(root: &Path, path: &Path) -> Result<(), String> {
    let relative = path
        .strip_prefix(root)
        .map_err(|_| "Restore path escapes application data".to_string())?;
    let mut current = root.to_path_buf();
    for part in relative.components() {
        current.push(part);
        match fs::symlink_metadata(&current) {
            Ok(metadata) if metadata.file_type().is_symlink() => {
                return Err(format!("Restore refuses symlink {}", current.display()))
            }
            Ok(_) => (),
            Err(error)
...
pub fn recover_interrupted_restores(app_dir: &Path, db: &Database) -> Result<(), String> {
    ...
    let entries =
        fs::read_dir(app_dir).map_err(|e| format!("Failed to inspect restore journals: {e}"))?;
    for entry in entries {
        let path = entry
            .map_err(|e| format!("Failed to inspect restore journal: {e}"))?
            .path();
        let Some(name) = path.file_name().and_then(|n| n.to_str()) else {
            continue;
        };
        if !name.starts_with(RESTORE_JOURNAL_PREFIX) || name.ends_with(".tmp") {
            continue;
        }
        reject_symlink_components(app_dir, &path)?;
        let journal: RestoreJournal = serde_json::from_slice(
            &fs::read(&path).map_err(|e| format!("Failed to read restore journal: {e}"))?,
        )
        ...
        let committed: bool = db
            ...
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM restore_commits WHERE journal_path = ?1)",
                [&path.to_string_lossy()],
                |row| row.get(0),
            )
            .map_err(|e| format!("Failed to inspect restore commit marker: {e}"))?;
        guard.active = true;
        let errors = if committed || rolled_back {
            guard.cleanup_committed()
        } else {
...
impl<'a> RestoreRollbackGuard<'a> {
    ...
    fn from_journal(
        ...
        let valid_dir = |path: &Path, prefix: &str| {
            path.starts_with(app_dir)
                && path.parent() == Some(app_dir)
                && path
                    .file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with(prefix))
        };
        if !valid_dir(&journal.staging_dir, ".restore_staging_")
            || !valid_dir(&journal.backup_dir, ".restore_backup_")
        {
            return Err(format!(
                "Restore journal {} contains unsafe directories",
                journal_path.display()
            ));
        }
        reject_symlink_components(app_dir, &journal.staging_dir)?;
        reject_symlink_components(app_dir, &journal.backup_dir)?;
        for rel_path in journal
            .replaced_assets
            .iter()
            .chain(journal.created_assets.iter())
        {
            ...
            {
                return Err(format!(
                    "Restore journal {} contains unsafe asset path",
                    journal_path.display()
                ));
            }
        ...
        Ok(Self {
            ...
            journal_path,
            replaced_assets: journal.replaced_assets,
            created_assets: journal.created_assets,
            active: false,
        })
    }
    ...
    fn journal(&self) -> RestoreJournal {
        RestoreJournal {
            ...
            replaced_assets: self.replaced_assets.clone(),
            created_assets: self.created_assets.clone(),
            committed: false,
            phase: "pending".into(),
        }
    }
    ...
    fn rollback_inner(&mut self) -> Vec<String> {
        ...
        for rel_path in &self.created_assets {
            ...
            if dest.exists() {
                if let Err(e) = fs::remove_file(&dest) {
                    errors.push(format!("remove {}: {e}", dest.display()));
                }
            }
        ...
        if errors.is_empty() {
            if let Err(e) = self.persist_phase("rolled_back") {
                errors.push(e);
            }
        }
        ...
        for (path, label) in [(&self.backup_dir, "backup"), (&self.staging_dir, "staging")] {
            if path.exists() {
                if let Err(e) = fs::remove_dir_all(path) {
                    errors.push(format!("remove {label} directory: {e}"));
                }
            }
        }
        if errors.is_empty() {
...
pub fn restore_from_backup(
    ...
    for asset in &archive.assets {
        if !archive.asset_files.contains_key(&asset.relative_path) {
            return Err(format!(
                "Backup is missing file data for asset {}",
                asset.id
            ));
        }
    }
    ...
    for anchor in &archive.note_source_anchors {
        ...
        .map_err(|e| {
            format!(
                "Failed to replace note source anchor for note {}: {e}",
                anchor.note_id
            )
        })?;
