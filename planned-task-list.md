...
- [x] **2.2 — Real PDF import and document ownership modes.**
  - Verification note (2026-08-06): Implemented in `src/utils/pdfImport.ts`, `src/components/ImportModal.tsx`, `src/components/MissingFileBanner.tsx`, and `src-tauri/src/import.rs`. Supports native file picker (`dialog:allow-open`) and HTML drag-drop import; pre-selects open-in-place as recommended default per ADR R0.8 / OQ-9 with explicit non-destructive selection; managed-library mode copies to `documents/` via Rust streaming without moving or modifying original file; checks SHA-256 fingerprint duplicates; implements missing file banner with "Locate File" recovery flow. Unit tested in `src/utils/pdfImport.test.ts` (7 Vitest tests) and `src-tauri/src/import.rs` (2 Rust tests).
